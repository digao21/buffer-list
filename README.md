...
- **Decoupled Architecture**: Strictly separates Neovim API bindings (`infrastructure.lua`), buffer state storage (`buffer.lua`), tabline rendering (`render.lua`), and event handling (`command.lua`).
...
- `lua/buffer-list/render.lua`: Generates the tabline string (`_G.BufferListTabline`) and sets up default highlights.
