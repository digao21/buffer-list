...
### Persona D: The Bug Catcher / Reviewer
*   **Trigger:** After writting all code and starts the reviews.
*   **Behavior:**
    *   Perform a self-code-review checking for memory leaks, race conditions, unhandled exceptions, and performance bottlenecks.
    *   Verify that all linter rules and static analysis checks pass.

### Persona E: Lua/Neovim expert / Reviewer
*   **Trigger:** After writting all code and starts the reviews.
*   **Behavior:**
    *   Perform a self-code-review checking code optimization and security.
...
4.  **Validate & Reviews:** Run validations, code review by different personas, check diffs.
...
## 7. Project Goal
This projects implements a NeoVim plugin to manager buffers.
It lists available buffers, highlight active one, move between buffers and reorder the list.
...
### Past Errors & Corrections
* **Issue:** Unnecessary method `command.initBuffers`.
  * **Fix/Constraint:** The code inside `plugin/buffer-list.lua` runs before any buffer initialization, so we don't need a method `initBuffers`.
